...
const ImprocedentesBKOTable = ({ data }) => {
  const [searchTerm, setSearchTerm] = useState('');

  const improcedentes = useMemo(() => data
    .filter(isImprocedenteBKO)
    .map(item => ({
      data: formatDateBR(item.dataFinalizacao) || item.dataFinalizacao || '-',
      protocolo: item.id || '-',
      observacaoEstado: item.observacaoEstado || item.observacao || 'Sem observação',
      tratadoPor: item.tratadopor || 'Não informado',
    })), [data]);
  ...
  const exportCSV = () => {
    ...
    const headers = ['Data', 'Protocolo', 'Observação Estado', 'Tratado por'];
    const rows = improcedentes.map(row => [
      row.data,
      row.protocolo,
      row.observacaoEstado,
      row.tratadoPor,
    ]);
    ...
    link.download = 'tickets_improcedentes_bko.csv';
    link.click();
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              {['Data', 'Protocolo', 'Observação Estado', 'Tratado por'].map(header => (
            ...
            {filteredRows.length === 0 ? (
              <tr>
                <td colSpan={4} className="text-center py-8 text-gray-500 dark:text-gray-400">
                  Nenhum ticket improcedente BKO encontrado
                </td>
              </tr>
            ) : (
              filteredRows.map((row, index) => (
                <tr key={`${row.protocolo}-${index}`} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors align-top">
                  <td className="py-3 px-3 text-xs text-gray-700 dark:text-gray-300 whitespace-nowrap">{row.data}</td>
                  <td className="py-3 px-3 text-xs font-semibold text-gray-800 dark:text-gray-100 tabular-nums">{row.protocolo}</td>
                  <td className="py-3 px-3 text-xs text-gray-700 dark:text-gray-300 max-w-[520px] whitespace-normal leading-relaxed">{row.observacaoEstado}</td>
                  <td className="py-3 px-3 text-xs text-gray-700 dark:text-gray-300">{row.tratadoPor}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default ImprocedentesBKOTable;
